...
In the original Bayesian derivation, $F_A$ approximates local curvature of the old-task posterior around $\theta_A^\star$ [Kirkpatrick et al.](https://arxiv.org/abs/1612.00796). In practical empirical-Fisher EWC, the same form is used with an empirical second moment of old-task gradients. Either way, the matrix is not merely a compression device. It is the local quadratic object through which old-task preservation is imposed.
