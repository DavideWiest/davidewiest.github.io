---
permalink: /
title: "Davide Wiest"
layout: homepage
author_profile: false
hero_intro: >-
  I am a computer science student at TU Darmstadt, interested in machine learning theory, systems, and research at the intersection of both. I am currently exploring underexplored parts of the training stack, especially practically applicable approaches that enhance the capabilities models acquire during training.
redirect_from: 
  - /about/
  - /about.html
---

## Endeavors and Interests

I study computer science at TU Darmstadt and have been here since winter 2025. My aim is a research-oriented career in machine learning, though I am still figuring out exactly which problems deserve the most attention. I want to understand ML on both a theoretical and practical level, which means reading papers, implementing ideas, and occasionally discovering why something that sounded elegant falls apart in practice.

## Previous Work

I learn by building, and some of those builds turned out worthwhile enough to keep around. [pptrain](/work/) is a PyTorch library for pre-pretraining language models on synthetic tasks. [TRL](/work/) is a local, self-supervised learning procedure that optimizes each neuron individually without backpropagation. [Gradient Routing for Continual Learning](/work/) tests whether sparsity can reduce catastrophic forgetting. [Closed-Form Initialization](/work/) explores whether neural networks can be initialized analytically. See [all work](/work/) for more.

## About Me

I work best late at night, which is either a genuine advantage or a convenient excuse, depending on whether the deadline is met. I listen to synthwave and phonk, go on long walks when I am stuck on a problem, and get genuinely excited by ideas that change how a field operates rather than marginally improving what already exists. Philosophically I lean toward anti-essentialism, naturalism, and structuralism (with bundle theory as a useful framing). In terms of how I operate, I want to get things done and be maximally pragmatic, but I also want to experiment and do creative things even when their purpose is not immediately clear.
